  ...
  return (
    <div>
        ...
        {catagory.map((buttonName) => (
            <button
            key={buttonName}
            className={`cursor-pointer px-2 py-2 font-semibold rounded-lg border-2 border-primary dark:border-primaryDark 
        ...
        className='grid grid-cols-3 gap-[1.8rem] text-dark justify-center pt-4 dark:text-light'>
        
            {filteredProjects.map(project=>(
                <div key={project.id} className='bg-[#e2d6d6] p-6 rounded-lg dark:bg-[#322f2f]'>
                <Image src={project.img} alt="demo" height={500} width={500} className='rounded-lg mb-4 h-[75%] w-[100%]' />
                <h3 className='mb-2 text-lg font-semibold'>{project.name}</h3>
                <button onClick={()=>handleClick(project)} className='flex w-25 text-primary text-sm font-semibold items-center gap-x-1 cursor-pointer hover:translate-x-1 dark:text-primaryDark'> Project Details <ArrowIcon className="h-4 w-4 text-primary dark:text-primaryDark"/> </button>
    ...
    <>
        ...
        <main className='w-full mb-16 flex flex-col items-center justify-center dark:text-light'>
            <Layout className='pt-16'>
                <AnimatedText text={"Creativity Outshines Facts!"} className='mb-16'/>
